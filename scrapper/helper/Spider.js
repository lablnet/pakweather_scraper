    ...
    async crawl_page(thread_name, page_url) {
        //if (this.queue.has(page_url)) {
            ...
            if (page) {
                ...
                console.log ("Feel Like Temp", feelLikeTemp);
                let wind =  "" ;//await getData(page, 'class', 'Wind--windWrapper--3Ly7c', 'text');
                const windDirection = await elem(page, 'class', 'Wind--windWrapper--3Ly7c');
                ...
                const parentElement = await elem(details, 'class', 'WeatherDetailsListItem--WeatherDetailsListItem--1CnRC');
                const _data = await parentElement.locator('div');
                // const dewPoint = await getData(_data.nth(3), 'data-testid', 'TemperatureValue', 'text');
                // const moonPhase = await getData(_data[_data.length - 1], 'default', '', 'text');
                // const temps = await _data[0].$$('data-testid', 'TemperatureValue');
                // let high = await getData(temps.nth(0), 'default', '', 'text');
                // let low = await getData(temps.nth(1), 'default', '', 'text');
